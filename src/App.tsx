...
function App() {
  ...
  const [busStationDetail, setBusStationDetail] = useState<
    BusInfoDetail[] | null
  >(null);

  const [direction, setDirection] = useState(false);

  const [selectedBusStationForDetail, setSelectedBusStationForDetail] =
    useState("");

  const [closestBusStations, setClosestBusStations] = useState<
    BusStationInfo[]
  >([]);
  ...
  const getUpcomingBuses = async () => {
    ...
    BusService.getUpcomingBuses(Number(busStationInfo.id))
      .then((response) => {
        if (response) {
  ...
  const getBusStationDetail = async (stationId: string, index: number) => {
    ...
    selectedBusStationForDetail === stationId + index
      ? setSelectedBusStationForDetail("")
      : setSelectedBusStationForDetail(stationId + index);
      ...
      if (toggleLocation && navigator.geolocation) {
        navigator.geolocation.getCurrentPosition(
          (position) => {
            ...
            BusService.getClosestBusStations(
              position.coords.latitude,
              position.coords.longitude
            )
              .then((closestBusStations: BusStationInfo[]) => {
    ...
    <div className="container mx-auto max-w-md bg-zinc-100 dark:bg-zinc-900 text-zinc-900 dark:text-zinc-100 flex flex-col space-y-5 p-5">
      ...
      {toggleLocation && closestBusStations.length > 0 ? (
        <div className="flex flex-col space-y-2">
          ...
          <select
            className="appearance-none px-3 py-2 bg-zinc-200 dark:bg-zinc-800 rounded-lg"
            disabled={isLoading}
            value={
              closestBusStations.find(
                (busStation) => busStation.id === busStationInfo.id
              )?.id || ""
            }
            onChange={(e) => {
              const selectedStation = closestBusStations.find(
                (busStation) => busStation.id === e.target.value
              );
      ...
      <div className="flex justify-between items-center">
        {!isLoading && lastUpdateTime && (
          <p className="text-xs">Son Güncelleme Zamanı: {lastUpdateTime}</p>
        )}
        {isLoading && <p className="text-sm">Yükleniyor...</p>}
        <button
          onClick={() => setToggleLocation(!toggleLocation)}
          className="px-3 py-3 bg-zinc-200 dark:bg-zinc-800 rounded-lg flex justify-center items-center"
        >
          {toggleLocation ? (
      ...
      <button
        className="px-3 py-2 bg-blue-200 dark:bg-blue-800 rounded-lg"
                ...
                <div className="flex flex-col bg-zinc-200 dark:bg-zinc-800 rounded-lg">
                  ...
                  >
                    <div className="max-w-[75%] flex flex-col space-y-1">
                      <div className="font-bold text-3xl">{bus.number}</div>
                      <div className="text-xs">{bus.description}</div>
                    </div>
                        ...
                        <div className="flex flex-col space-y-2">
                          ...
                          <div className="flex justify-between items-center text-xs p-3 rounded-lg text-zinc-600 dark:text-zinc-400 bg-zinc-300 dark:bg-zinc-700">
                            <div className="flex items-center space-x-1">
                              ...
                              <span>Durak Konumu</span>
                            </div>
                            <div className="flex items-center space-x-1">
                              <span>Durak Kodu</span>
                              <FontAwesomeIcon icon={faHashtag} />
                            </div>
                          </div>
                          {busStationDetail?.[direction ? 0 : 1].stations.map(
                            (station, index) => (
                              <div
                                key={index}
      ...
      <div className="flex justify-center items-center mt-5">
        <p className="text-sm">
          ...
          <a
            ...
            href="https://linkedin.com/in/mustafayurdakul"
            target="_blank"
            rel="noreferrer"
          >
            ben
          </a>{" "}
          yaptım. Kaynak kodlarına{" "}
          <a
            className="text-blue-500"
            href="https://github.com/mustafayurdakul/when-bus"
            target="_blank"
            rel="noreferrer"
          >
            GitHub
